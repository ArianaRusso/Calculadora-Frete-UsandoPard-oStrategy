 ...
 */
public class FreteNormal implements CalculadoraDeFrete {
    @Override
    public float calcularFrete(float valorPedido, Integer distancia, Integer pesoTotalPedido) {

        float valorFrete = 0f;

        if (valorPedido > 200 || distancia < 10) valorFrete = 0;
        else if (distancia > 10 && distancia < 100) valorFrete = 20f;
        else if (distancia > 100) valorFrete = 40f;
        if (pesoTotalPedido > 5) valorFrete *= 1.5;
